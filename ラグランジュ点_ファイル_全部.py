# -*- coding: utf-8 -*-
"""
Created on Wed Jan 24 03:46:25 2018
...
#for x_base, _name in zip(Ls, ["L{}".format(_) for _ in range(1,6)]):
for x_base, _name in zip([L3], ["L3"]):
#x_base = L5
    for rotate, rot_label in zip([False], ["rot"]):
    ...
    #                rotate = True
                    ...
                    yield t, _x, _r0, _r1, _x_base
        #            print(t)
        g = gen()
                  #%%
        x_hist = []
        for i in range(1000):
            t, _x, _r0, _r1, _x_base = next(g)
            x_hist.append([t, _x, _r0, _r1, _x_base])
            print(i)
            
#%%
        def func(data):
            t, _x, _r0, _r1, _x_base = data
            ax.cla()
            ax.set_aspect("equal")
            
            ax.set_title("t = {:4.1f}".format(t))
            ax.set_xlim(-nmax,nmax)
            ax.set_ylim(-nmax,nmax)
            
            ax.scatter(_x[0],_x[1], s=0.5, marker=".")
        ...
        ani = animation.FuncAnimation(fig, func, x_hist, interval=30)
        with open("003_{}_{}.html".format(_name, rot_label), "w") as fp:
            fp.write(ani.to_html5_video())
#plt.show()
...
#    ax.set_ylim(-nmax,nmax)
#    
#    ax.scatter(_x[0],_x[1], s=3)
#    ax.scatter(_x_base[0],_x_base[1], c="r", s=30)
#    ax.scatter(_r0[0],_r0[1], c="b", s=30)
#    ax.scatter(_r1[0],_r1[1], c="b", s=30)
#    
